...
## Consequences
`backend/src/api/main.py` wires routers per ../specs/api-contract.md; `uvicorn` single worker (shared in-memory CRDT registry — do NOT scale workers at demo).
